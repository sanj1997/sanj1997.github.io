...
const Skills=()=>{
    ...
    return(
        <Box padding={"50px"} bg="#2c1338" id="skills">
            <Text textAlign={"center"} fontSize={"5xl"} color={"#fff3ed"}><Highlight query={"Technical"} styles={{color:"#e57cd8"}}>My Technical Skills</Highlight></Text>
            <Flex marginTop={"30px"} flexDirection={"column"} justifyContent="space-evenly" alignItems={"center"}>
                ...
                <Box marginTop={"50px"} display={"grid"} gridTemplateColumns={["repeat(1,1fr)","repeat(2,1fr)","repeat(2,1fr)","repeat(4,1fr)","repeat(6,1fr)"]} gap={["15px","20px","30px","50px"]} textAlign={"center"} >
                     {req.map((el)=>{
                        return <>
                        <Flex cursor={"pointer"} borderRadius={"22px"} flexDirection={"column"} bg="#f7f7f7" justifyContent={"space-between"} padding="10px" height={"150px"}>
